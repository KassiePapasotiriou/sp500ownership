...
## Yearly average ownership by industry
...
ggplot(cleveland, aes(x = avg_users_holding, y =fct_reorder2(Symbol, Year == '2020', 
    ...
    ylab("Industry\n") +
    ggtitle("Average yearly stock solding by industry\nAugust 2018 - August 2020") + theme_gray(16)
```
...
df2 %>% filter(Symbol %in% very_pop) %>%  
  ...
  ylab ("Number of accounts who wwn the stock") + 
...
tidydf %>%
  ggplot( aes(x=Date, y=index, group=Symbol, color=Symbol)) +
  geom_line(size=0.8) +
  geom_vline(xintercept =ymd("2020-01-21"), color ="blue") +
  annotate("text", x =ymd("2020-01-21"), y =30.75,label =" Fist COVID-19 case in the US\n 1/21/2020", color ="blue",hjust =0, size = 7) +
  scale_x_date(date_labels = "%b-%Y", breaks = "2 month") +
...
tidydf2 %>%
  ...
  scale_x_date(date_labels = "%b-%Y", breaks = "2 month") + 
  ggtitle("Daily normalized Market Cap for the most popular S&P500 companies\nAugust 2018 - August 2020")  +
  xlab("") +
  ylab("Market Cap") +  
  scale_y_continuous(label=addUnits) +
  theme(plot.title=element_text(size=30),
...
* When comparing the old and the medium founding range companies (`1784-1892`and `1894-1952`) there seems to be a positive correlation with popularity. Companies that were founded in `1894-1952` are more likely to tbelong to the `normal` and the `very_popular` categories compared to companies founded in `1784-1892`.

* However, this positive correlation trend is broken when we look at the most recently founded companies. This group contains companies that have the most `normal` popularity companies across all foudning ranges groups
